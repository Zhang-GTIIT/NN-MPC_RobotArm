...
class ASAPPlannerWorker(threading.Thread):
    ...
    def run(self) -> None:
        try:
            ...
            rollout = PlannerRolloutConfig(mpc_policy="residual", q_ref_velocity_limit=t(velocity_limit), q_ref_acceleration_limit=t(acceleration_limit), residual_max=t(residual_max), joint_limit_margin=self.args.joint_limit_margin, rollout_batch_size=self.args.rollout_batch_size, project_residual_kinematics=self.args.planner_projection == "on", residual_cost_semantics=self.args.residual_cost_semantics, residual_feasibility_semantics=self.args.residual_feasibility_semantics)
