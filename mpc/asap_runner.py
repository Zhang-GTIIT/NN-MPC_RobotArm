...
def run(args: Any, api: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for step in range(execution_steps):
            ...
            if active is not None:
                ...
                if index is not None:
                    ...
                    if active.requested_residual_sequence.shape == active.residual_sequence.shape:
                        planner_requested = active.requested_residual_sequence[index].copy()
                    else:
            ...
            command, executed, velocity = project_executable_command_np(nominal, requested_correction, previous_command, previous_velocity, env.joint_low, env.joint_high, args.joint_limit_margin, physical_v, physical_a, env.control_dt)
            projection_discrepancy = (requested_correction - executed).astype(np.float32)
